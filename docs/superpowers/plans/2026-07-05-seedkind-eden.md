...
- Create `src/SeedKindEden.jsx`: Eden UI state, seed packet selection, copy controls, return label form, parser feedback, next seed display, and branch invitation.
...
- Modify `tests/launch-hosts.spec.js`: assert Eden CTA, growth ladder, seed packet copy, consent gating, valid return seed next seed, and mobile overflow.
...
- a copyable current seed packet generated by `composeSeedPrompt`
- a return label form with returned seed label, consent checkbox, optional name/email, optional 1-5 feeling fields, optional efficacy note, and contact permission
- parser feedback using `parseReturnSeed`
- next seed display using `composeNextPrompt`
...
- `Copy Seed Packet` button is visible
- return form blocks parsing without consent
- a valid `RETURN_SEED_V1` produces a next seed
...
- Spec coverage: Eden CTA, ladder, seed packets, return seed parser, next seed, branch invitation, optional survey, safety copy, consent-first form, tests, docs.
